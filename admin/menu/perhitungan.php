		...
		----------------------------------*/
			...
			?>
				...
				?>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="datatable-5 table table-bordered table-striped	 display" style="margin-bottom: 50px;" width="100%">
						<thead>
							<tr>
								<th>Pelamar</th>
								<th>Kriteria</th>
								<th>Nilai</th>
								<th>Bobot dipangkatkan</th>
								<th>Vektor S</th>
								<!-- <th></th> -->
							</tr>
						</thead>
						...
						<tbody>
							...
							foreach ($ar_pelamar as $key => $item) :
								$loop2 = $pelamar->GetData("where id_lowongan='$id_lowongan' and id_user='".$item['id_user']."' and nilai > 0 order by kriteria asc");
								$ib = 0;
								$vkt_s = 1;
							?>
								<tr>
									<td rowspan="<?= $no ?>"><?= $item['id_user'] ?></td>
								</tr>
								...
								<?php
									...
									$ib++;
								endwhile;
								$qryVS = $hitung->SetVektor_S($vkt_s, $item['id_user'], $id_lowongan);
								$qryVV = $hitung->SetVektor_V($item['id_user'], $id_lowongan);
								?>
							<?php
							endforeach;
							?>
